...
app.get("/keys", async (_, res) => {
  const key = await jose.JWK.asKey(config.privateKeyAsPem, "pem", {
    alg: "RS256",
    key_ops: ["verify"],
  });
...
app.use("/env.js", (_, res) => {
  const out = {
    ...
    PICKER_ORIGIN:
      process.env.PICKER_ORIGIN || "https://patient-browser.smarthealthit.org",
    GOOGLE_ANALYTICS_ID: process.env.GOOGLE_ANALYTICS_ID,
    FHIR_SERVER_R2: config.fhirServerR2,
    FHIR_SERVER_R3: config.fhirServerR3,
    FHIR_SERVER_R4: config.fhirServerR4,
    ACCESS_TOKEN: jwt.sign({ client_id: "launcherUI" }, config.jwtSecret, {
      expiresIn: "10 years",
    }),
    VERSION: pkg.version,
...
if (require.main?.filename === __filename) {
  const server = app.listen(+config.port, config.host, () => {
    ...
    console.log(
      `SMART launcher available at http://${address.address}:${address.port}`,
    );
  });
    ...
    require("pem").createCertificate(
      ...
      (err: Error, keys: any) => {
        ...
        require("https")
          ...
          .listen(process.env.SSL_PORT, config.host, () => {
            const address = server.address() as AddressInfo;
            console.log(
              `SMART launcher available at https://${address.address}:${address.port}`,
            );
          });
      },
    );
