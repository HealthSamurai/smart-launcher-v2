...
export function notSupported(message: string = "", code = 400) {
  return (req: Request) => {
    throw new HttpError(
      message ||
        `${req.method} ${req.originalUrl} is not supported by this server`,
    ).status(code);
  };
}
...
export function getFhirServerBaseUrl(req: Request) {
  ...
  let fhirServer = config[
    `fhirServer${fhirVersion}` as keyof typeof config
  ] as string;
  ...
  if (process.env["FHIR_SERVER_" + fhirVersion + "_INTERNAL"]) {
    fhirServer = process.env[
      "FHIR_SERVER_" + fhirVersion + "_INTERNAL"
    ] as string;
  }

  if (!fhirServer) {
    throw new HttpError(
      'FHIR server "%s" not found',
      req.params.fhir_release,
    ).status(400);
  }
