...
export default class TokenHandler {
    ...
    switch (req.body.grant_type) {
      ...
      default:
        throw new OAuthError(
          'Invalid or missing grant_type parameter "%s"',
          req.body.grant_type,
        )
  ...
  public async handleBackendServices(): Promise<void> {
    const { scope, client_assertion, client_assertion_type } =
      this.request.body;
    ...
    // Validate client_id
    if (
      launchOptions.client_id &&
      token?.payload.sub !== launchOptions.client_id
    ) {
      throw new InvalidClientError(
    ...
    if (invalidScopes) {
      throw new InvalidScopeError(
        `Invalid scope(s) "%s" requested. Only system scopes are allowed.`,
        invalidScopes,
      ).status(401);
    }

    // Basic scope negotiation
    let grantedScopes = client.scope
      ? new ScopeSet(client.scope).negotiate(scope).grantedScopes
      : scope.trim().split(/\s+/);
    ...
    // access_token
    tokenResponse.access_token = jwt.sign(accessToken, config.jwtSecret, {
  ...
  public async handleAuthorizationCode(): Promise<void> {
    ...
    if (!redirect_uri) {
      throw new InvalidRequestError("Missing 'redirect_uri' parameter").status(
        400,
      );
    }
    ...
    // Verify code
    try {
      var authorizationToken = jwt.verify(
        code,
        config.jwtSecret,
      ) as SMART.AuthorizationToken;
    } catch (ex) {
      throw new InvalidClientError(
        "Invalid token (supplied as code parameter in the POST body). %s",
        (ex as Error).message,
      ).status(401);
    }
    ...
    if (authorizationToken.redirect_uri !== redirect_uri) {
      throw new InvalidRequestError("Invalid redirect_uri parameter").status(
        401,
      );
    }
    ...
    if (
      ...
      authorizationToken.pkce === "always"
    ) {
      ...
      if (!code_verifier) {
        throw new InvalidRequestError("Missing code_verifier parameter").status(
          400,
        );
      }
   ...
   */
  public async validateClientAssertion(
    clientAssertion: string,
    client: Partial<SMART.AuthorizationToken>,
  ): Promise<void> {
    // client_assertion must be a token ------------------------------------
    try {
      var { header: jwtHeaders, payload: token } = jwt.decode(clientAssertion, {
        complete: true,
        json: true,
      }) as {
        header: JwtHeader;
    ...
    // ---------------------------------------------------------------------
    const key = await this.pickPublicKey(
      unionJWKS.keys,
      jwtHeaders.kid,
      jwtHeaders.alg,
    );
    ...
    try {
      jwt.verify(clientAssertion, key.toPEM(), {
    ...
    if (jwtHeaders.jku && jwtHeaders.jku !== client.jwks_url) {
      throw new InvalidRequestError(
        "Invalid jku header of the assertion token token. must be '%s'",
        client.jwks_url,
      ).status(401);
    }
  }
  ...
  protected async pickPublicKey(keys: JsonWebKey[], kid: string, alg: string) {
    ...
    if (!_keys.length) {
      throw new InvalidClientError(
        "None of the keys found in the JWKS have alg equal to %s",
        alg,
      ).status(401);
    }
    ...
    if (!_keys.length) {
      throw new InvalidClientError(
        "None of the keys found in the JWKS have kid equal to %s",
        kid,
      ).status(401);
    }
    ...
    try {
      ...
      throw new InvalidClientError(
        "No usable public key found in the JWKS. %s",
        ex,
      ).status(401);
    }
  }
  ...
  public handleRefreshToken(): void {
    try {
      var token: any = jwt.verify(
        this.request.body.refresh_token,
        config.jwtSecret,
      );
    } catch (ex) {
      throw new OAuthError("Invalid refresh token")
        .errorId("invalid_grant")
        .status(401);
    }

    if (token.auth_error === "token_expired_refresh_token") {
      throw new OAuthError("Expired refresh token")
        .errorId("invalid_grant")
        .status(403);
    }
  ...
  public validateBasicAuth(authorizationToken: SMART.AuthorizationToken): void {
    ...
    if (secret) {
      ...
      if (!auth) {
        throw new InvalidRequestError(
          "The authorization header '%s' cannot be empty",
          authHeader,
        ).status(401);
      }
      ...
      if (
        authorizationToken.client_id &&
        authorizationToken.client_id !== auth[0]
      ) {
        throw new InvalidClientError(
  ...
  public finish(authorizationToken: SMART.AuthorizationToken) {
    ...
    if (authorizationToken.auth_error === "token_invalid_token") {
      throw new InvalidClientError("Simulated invalid client error").status(
        401,
      );
    }
    ...
    const tokenResponse: SMART.AccessTokenResponse = {
      ...
      id_token:
        ...
        (scope.has("profile") || scope.has("fhirUser"))
          ...
          : undefined,
      refresh_token:
        scope.has("offline_access") || scope.has("online_access")
