...
export default function AuthorizeLaunch() {
  ...
  ["read", "search", "create", "update", "delete", "write", "other"].forEach(
    (key) => {
      groups[key].forEach((msg, i) => {
        listItems.push(
          <li key={key + "-" + i} dangerouslySetInnerHTML={{ __html: msg }} />,
        );
      });
    },
  );
  ...
  if (!listItems.length) {
    return (
      <p>
        <big>
          Do you want to launch this application? No useful scopes requested!
        </big>
      </p>
    );
  }
  ...
  function submit(approve?: boolean) {
    const url = new URL(
      searchParams.get("aud")!.replace(/\/fhir$/, "/auth/authorize"),
    );
          ...
          <div className="col-xs-12 col-md-offset-1 col-md-10">
            {groups.access.length > 0 && (
              <div className="alert alert-warning access-alert">
                <b
                  className="glyphicon glyphicon-info-sign pull-left"
                  style={{ margin: "7px -7px -7px 7px" }}
                />
                <ul id="access-note">
                  {groups.access.map((msg, i) => (
...
function scopeToText(scope: string, isPatient?: boolean) {
  ...
  switch (scope) {
    case "smart/orchestrate_launch":
      return {
        ...out,
        other:
          '<i class="glyphicon glyphicon-ok-sign text-warning"></i>&nbsp;<b class="text-warning">Launch</b> other SMART applications',
      };
    case "profile":
    case "fhirUser":
      return {
        ...out,
        read: '<i class="glyphicon glyphicon-ok-sign text-success"></i>&nbsp;<b class="text-success">Read</b> our profile information',
      };
    case "launch":
      return {
        ...out,
        read: '<i class="glyphicon glyphicon-ok-sign text-success"></i>&nbsp;<b class="text-success">Read</b> all data about the selected patient and encounter',
      };
    case "launch/patient":
      return {
        ...out,
        read: '<i class="glyphicon glyphicon-ok-sign text-success"></i>&nbsp;<b class="text-success">Read</b> all data about the selected patient',
      };
    case "launch/encounter":
      return {
        ...out,
        read: '<i class="glyphicon glyphicon-ok-sign text-success"></i>&nbsp;<b class="text-success">Read</b> all data about the selected encounter',
      };
    case "online_access":
      return {
        ...out,
        access:
          "The application will be able to access data while you are online (<code>online access</code>) without having to be re-launched when its access token expires.",
      };
    case "offline_access":
      return {
        ...out,
        access:
          "The application will be able to access data until you revoke permission (<code>offline access</code>).",
      };
  }
  ...
  if (
    ...
    if (!isPatient) {
      text =
        scopeParts[1] === "*" ? "all" : "<code>" + scopeParts[1] + "</code>";
    ...
    if (scopeParts[2] === "write" || scopeParts[2] === "*") {
      out.write =
        '<i class="glyphicon glyphicon-ok-sign text-danger"></i>&nbsp;<b class="text-danger">Write</b> ' +
        text;
    }

    if (scopeParts[2] === "read" || scopeParts[2] === "*") {
      out.read =
        '<i class="glyphicon glyphicon-ok-sign text-success"></i>&nbsp;<b class="text-success">Read</b> ' +
        text;
    }
