...
function reducer(
  state: EncounterPickerState,
  action: PatientPickerAction,
): EncounterPickerState {
  switch (action.type) {
    case "setData":
...
async function fetchPatient(
  ...
  if (!res.ok) {
    let msg = txt;
    try {
      msg = JSON.stringify(JSON.parse(txt), null, 4);
    } catch {}
    throw new Error(msg);
  }
...
async function fetchEncounters(
  ...
  signal: AbortSignal,
  pageSize = 10,
): Promise<fhir4.Bundle<Encounter>> {
...
export default function EncounterPicker() {
  ...
  useEffect(() => {
    if (!patient) {
      dispatch({
        type: "setError",
        payload: '"patient" parameter is required!',
      });
      return;
    }
    ...
    if (Array.isArray(patient)) {
      dispatch({
        type: "setError",
        payload: '"patient" parameter must be provided exactly once!',
      });
      return;
    }
  ...
  return (
    <HelmetProvider>
      ...
      <div
        className="container-fluid encounter-picker"
        style={{ maxWidth: "80em" }}
      >
        {error && (
          <div className="row">
            ...
            <div className="col-sm-12">
              <pre className="alert alert-danger">
                ...
                <span>
                  {String(
                    error ||
                      "An error occurred loading patient encounters. Please try again later.",
                  )}
                </span>
              </pre>
            </div>
          ...
          <div className="col-sm-12">
            <table className="table mb-0">
              ...
              <tbody>
                ...
                {data?.entry?.map(
                  (p: fhir4.BundleEntry<fhir4.Encounter>, i) => (
                    <tr
                      key={i}
                      tabIndex={0}
                    ...
                    >
                      ...
                      <td>
                        <EncounterType encounter={p.resource!} />
                      </td>
                      <td>
                        {p.resource!.reasonCode?.[0].coding?.[0].display || (
                          <span className="text-muted">-</span>
                        )}
                      </td>
                      <td>
                        {getEncounterClass(p.resource!) || (
                          <span className="text-muted">-</span>
                        )}
                      </td>
                      <td>
                      ...
                      <td>
                        {p.resource?.period && (
                          <Period period={p.resource.period} />
                        )}
                      </td>
                    </tr>
                  ),
