        ...
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          {!!(
            ...
            !(
              ...
              validationErrors[0] === "Missing app launch URL"
            )
          ) && (
            <div
          ...
          >
            ...
            <div style={{ display: "flex" }}>
              <div style={{ flex: "10 1 0" }}>
                <div className="input-group">
                  <input
                    ...
                    value={
                      isStandaloneLaunch || isBackendService ? aud : launch_url
                    }
                    onChange={(e) =>
                      !isStandaloneLaunch &&
                      !isBackendService &&
                      setQuery({ launch_url: e.target.value })
                    }
                  ...
                  <span className="input-group-btn">
                    {isStandaloneLaunch || isBackendService ? (
                      <button
                        ...
                        onClick={() => copyElement("#launch-url")}
                      >
                        Copy
                      </button>
                    ) : validationErrors.length ? (
                      ...
                      <a
                        ...
                        className={
                          "btn btn-primary" +
                          (validationErrors.length ? " disabled" : "")
                        }
                      >
                        Launch
                      </a>
              ...
              <div style={{ flex: "1 1 0", marginLeft: 5 }}>
                {validationErrors.filter(
                  (e) =>
                    e !== "Missing app launch URL" &&
                    e !== "Invalid app launch URL",
                ).length ? (
                  <button className="btn btn-default" disabled>
                    Launch Sample App
                  </button>
            ...
            {isStandaloneLaunch || launch_type === "backend-service" ? (
              <span className="small text-muted">
                Your app should use this url to connect to the sandbox FHIR
                server
              </span>
            ) : (
              <span className="small text-muted">
                Full url of the page in your app that will initialize the SMART
                session (often the path to a launch.html file or endpoint)
              </span>
            )}
          </div>
          <br />
        ...
        <p className="text-center small">
          ...
          {window.ENV.COMMIT && (
            <>
              ...
              <a
                href={
                  "https://github.com/smart-on-fhir/smart-launcher-v2/commit/" +
                  window.ENV.COMMIT
                }
                rel="noreferrer noopener"
                target="_blank"
...
function LaunchTab() {
  ...
  return (
    <div className="row">
      <div
        className={
          launch.launch_type === "backend-service" ? "col-xs-12" : "col-sm-6"
        }
      >
        <div className="form-group">
          <label htmlFor="launch_type" className="text-primary">
            Launch Type
          </label>
          <select
            name="launch_type"
            ...
            onChange={(e) =>
              setQuery({ launch_type: e.target.value as SMART.LaunchType })
            }
          >
            {launchTypes.map((o) => (
          ...
          <span className="help-block small">
            {
              launchTypes.find((l) => l.value === launch.launch_type)
                ?.description
            }
          </span>
        </div>
        ...
        <div className="row">
          <div className="col-md-6">
            <div className="form-group">
              <label htmlFor="fhir_version" className="text-primary">
                FHIR Version
              </label>
              <select
                name="fhir_version"
                ...
                onChange={(e) =>
                  setQuery({
                    fhir_version: e.target.value,
                    patient: "",
                    provider: "",
                  })
                }
              >
                <option value="r4" disabled={!ENV.FHIR_SERVER_R4}>
                  R4
                </option>
                <option value="r3" disabled={!ENV.FHIR_SERVER_R3}>
                  R3 (STU3)
                </option>
                <option value="r2" disabled={!ENV.FHIR_SERVER_R2}>
                  R2 (DSTU2)
                </option>
              </select>
              <span className="help-block small">
          ...
          <div className="col-md-6">
            <div className="form-group">
              <label htmlFor="sim_error" className="text-primary">
                Simulated Error
              </label>
              <select
                id="sim_error"
                ...
                onChange={(e) =>
                  setQuery({
                    auth_error: e.target.value as SMART.SimulatedError,
                  })
                }
              >
                ...
                {launch.launch_type !== "backend-service" && (
                  <optgroup label="During the authorize request">
                    <option value="auth_invalid_client_id">
                      invalid client_id
                    </option>
                    <option value="auth_invalid_redirect_uri">
                      invalid redirect_uri
                    </option>
                    <option value="auth_invalid_scope">invalid scope</option>
                    <option value="auth_invalid_client_secret">
                ...
                <optgroup label="During the token request">
                  ...
                  <option value="token_expired_registration_token">
                    expired token
                  </option>
                  {launch.launch_type !== "backend-service" && (
                    <option value="token_expired_refresh_token">
                      expired refresh token
                    </option>
                  )}
                  <option value="token_invalid_scope">invalid scope</option>
                  {launch.launch_type === "backend-service" && (
                    <option value="token_invalid_jti">invalid jti</option>
                  )}
                </optgroup>

                <optgroup label="During FHIR requests">
                  <option value="request_invalid_token">
                    invalid access token
                  </option>
                  <option value="request_expired_token">
                    expired access token
                  </option>
                </optgroup>
              </select>
              <span className="help-block small">
                Force the server to throw certain type of error (useful for
                manual testing).
              </span>
            </div>
          </div>
        ...
        {launch.launch_type !== "backend-service" && (
          <div className="form-group">
            ...
            {(launch.launch_type === "provider-ehr" ||
              launch.launch_type === "patient-portal") && (
              <div className="checkbox">
                <label>
                  ...
                  />{" "}
                  Simulate launch within the EHR UI{" "}
              ...
              <div className="checkbox">
                <label>
                  <input
                    ...
                    disabled={
                      (launch.launch_type === "patient-portal" &&
                        (launch.patient || "").split(",").filter(Boolean)
                          .length !== 1) ||
                      (launch.launch_type === "provider-standalone" &&
                        (launch.provider || "").split(",").filter(Boolean)
                          .length !== 1)
                    }
      ...
      {launch.launch_type !== "backend-service" ? (
        <div className="col-sm-6">
          <div className="form-group">
            <label htmlFor="patient" className="text-primary">
              Patient(s)
            </label>
            <PatientInput
            ...
            />
            <span className="help-block small">
              Simulates the active patient in EHR when app is launched. If no
              Patient ID is entered or if multiple comma delimited IDs are
              specified, a patient picker will be displayed as part of the
              launch flow.
            </span>
          </div>

          {launch.launch_type !== "patient-portal" &&
            launch.launch_type !== "patient-standalone" && (
              <div className="form-group">
                ...
                <span className="help-block small">
                  ...
                  selected, or if multiple comma delimited Practitioner IDs are
                  specified, a login screen will be displayed as part of the
                  launch flow.
                </span>
              </div>
            )}
          ...
          <div className="form-group">
            <label htmlFor="encounter" className="text-primary">
              Encounter
            </label>
            <select
              name="encounter"
            ...
            >
              ...
              <option value="MANUAL">
                Manually select an encounter if available
              </option>
            </select>
            <span className="help-block small">
...
function ValidationTab() {
      ...
      <div className="form-group">
        ...
        <div className="radio">
          <label>
            <input
              type="radio"
              checked={launch.client_type === "public"}
              onChange={() => setQuery({ client_type: "public" })}
            />{" "}
            <b>Public</b>
            <div className="small text-muted">
              for client-side web apps that cannot reliably keep a secret
            </div>
          </label>
        </div>
        <div className="radio">
          <label>
            <input
              type="radio"
              checked={launch.client_type === "confidential-symmetric"}
              onChange={() =>
                setQuery({ client_type: "confidential-symmetric" })
              }
            />{" "}
            <b>Confidential Symmetric</b>
            <div className="small text-muted">
              for clients using a client secret to authenticate
            </div>
          </label>
        </div>
        <div className="radio">
          <label>
            <input
              type="radio"
              checked={launch.client_type === "confidential-asymmetric"}
              onChange={() =>
                setQuery({ client_type: "confidential-asymmetric" })
              }
            />{" "}
            <b>Confidential Asymmetric</b>
            <div className="small text-muted">
              for clients authenticating with signed JWT
            </div>
          </label>
        </div>
      </div>
  ...
  function renderValidationWidget() {
    return (
      <div
        className="form-group pt-1 pb-2"
        style={{ borderBottom: "1px dashed #DDD" }}
      >
        <div className="row">
          <div className="col-xs-7">
            <label className="text-primary form-control-static">
            ...
            <div className="btn-group">
              <div className="btn-group">
                <button
                  ...
                  className={
                    "btn" +
                    (validation === "0" ? " btn-danger info" : " btn-default")
                  }
                  onClick={() => setValidation("0")}
                >
                  Loose
                </button>
              </div>
      ...
      <div className="form-group pt-1">
        ...
        {query.validation === "1" && (
          <div className="text-muted small">
            {launch.pkce === "none" && (
              <span>Do not require or validate PKCE</span>
            )}
            {launch.pkce === "auto" && (
              <span>
                Require PKCE if <code>code_challenge_method</code> parameter is
                sent
              </span>
            )}
            {launch.pkce === "always" && (
              <span>Require apps to use valid PKCE</span>
            )}
          </div>
        )}
      </div>
  ...
  function renderJWKSWidget() {
    return (
      <div className="form-group">
        <label htmlFor="jwks" className="text-primary">
          JWKS
        </label>
        <textarea
        ...
        />
        <span className="help-block small">
          Enter a valid JSON object listing your public JWK keys. Example:{" "}
          <code dangerouslySetInnerHTML={{ __html: '{"keys":[JWK(s)...]}' }} />.
          If provided, your client assertion must be signed with a public key
          from this set.
        </span>
      </div>
    );
  ...
  function renderInfoWidget() {
    return (
      <div
        className="form-group radio text-muted mb-0"
        style={{
          padding: "10px 15px",
          background: "#e7f4ff",
          borderRadius: 5,
      ...
      >
        <div>
          <label>
            <i
              className="fa-regular fa-circle-check text-primary"
              style={{ marginLeft: -20 }}
            />{" "}
            Your app can use any <b className="text-primary">client_id</b>
          </label>
        </div>
        <div>
          <label>
            <i
              ...
              style={{ marginLeft: -20 }}
            />{" "}
            Your app can request any{" "}
            {launch.launch_type === "backend-service" ? (
              <b className="text-primary">system</b>
            ) : (
              ""
            )}{" "}
            <b className="text-primary">scope</b>
          </label>
        </div>
        {launch.launch_type !== "backend-service" && (
          <div>
            <label>
              <i
                ...
                style={{ marginLeft: -20 }}
              />{" "}
              Your app can use any <b className="text-primary">redirect_uri</b>
            </label>
          </div>
        )}
        {(launch.launch_type === "backend-service" ||
          ...
          <div>
            <label>
              <i
                ...
                style={{ marginLeft: -20 }}
              />{" "}
              Your app can use any structurally valid JWT assertion
            </label>
          </div>
        )}
        {launch.launch_type !== "backend-service" &&
            ...
            <div>
              <label>
                <i
                  ...
                  style={{ marginLeft: -20 }}
                />{" "}
                Your app can use any{" "}
                <b className="text-primary">client_secret</b>
              </label>
            </div>
          )}
          ...
          <div>
            <label>
              <i
                ...
                style={{ marginLeft: -20 }}
              />{" "}
              Your app is not required to use PKCE
            </label>
          </div>
        )}
          ...
          <div>
            <label>
              <i
                ...
                style={{ marginLeft: -20 }}
              />{" "}
              PKCE is validated if{" "}
              <b className="text-primary">code_challenge_method</b> parameter is
              sent
            </label>
          </div>
        )}
            ...
            <div>
              <label>
                <i
                  ...
                  style={{ marginLeft: -20 }}
                />{" "}
                Your app is required to use valid PKCE
              </label>
            </div>
          )}
      </div>
  ...
  function renderClientIdWidget() {
    return (
      <div className="form-group">
        <label htmlFor="client_id" className="text-primary">
          Client ID
        </label>
        <input
          type="text"
        ...
        />
        <span className="help-block small">
          If you provide a <b>Client ID</b>, your <code>client_id</code> at
          runtime must match
        </span>
      </div>
    );
  ...
  function renderScopeWidget() {
    return (
      <div className="form-group">
        <label htmlFor="scope" className="text-primary">
          Allowed Scopes
        </label>
        <input
          type="text"
        ...
        />
        <span className="help-block small">
          Space-separated list of scopes that your app is allowed to request. If
          provided, your <code>scope</code> at runtime must be covered by this
          set.{" "}
          {launch.launch_type === "backend-service" && (
            <span>
              Only <code>system</code> scopes are supported for backend
              services.
            </span>
          )}
        </span>
  ...
  function renderRedirectUriWidget() {
    return (
      <div className="form-group">
        <label htmlFor="redirect_uris" className="text-primary">
  ...
  function renderClientSecretWidget() {
    return (
      <div className="form-group">
        <label htmlFor="client_secret" className="text-primary">
          Client Secret
        </label>
        <input
          type="text"
  ...
  function renderJwksUrlWidget() {
    return (
      <div className="form-group">
        <label htmlFor="jwks_url" className="text-primary">
          JWKS URL
        </label>
        <input
          type="url"
        ...
        />
        <span className="help-block small">
          Enter the URL where the JWKS with your public keys can be found. If
          provided, your client assertion must be signed with a public key from
          this set.
        </span>
      </div>
    );
  ...
  return (
    <>
      {launch.launch_type === "backend-service" ? (
        <div className="row">
          <div className="col-sm-12">{renderValidationWidget()}</div>
        </div>
      ) : (
        <div className="row">
          <div className="col-sm-6">{renderClientTypeWidget()}</div>
          <div className="col-sm-6">
          ...
          <div className="row">
            {launch.launch_type === "backend-service" ? (
              ...
              <>
                <div
                  className={
                    launch.client_type === "public"
                      ? "col-sm-12 mb-1"
                      : "col-sm-6"
                  }
                >
                  {renderRedirectUriWidget()}
                </div>
                {launch.client_type === "confidential-symmetric" && (
                  <div className="col-sm-6">{renderClientSecretWidget()}</div>
                )}
                {launch.client_type === "confidential-asymmetric" && (
                  <div className="col-sm-6">{renderJwksUrlWidget()}</div>
                )}
                {launch.client_type === "confidential-asymmetric" && (
                  <div className="col-sm-12 mb-1">{renderJWKSWidget()}</div>
                )}
              </>
            )}
          </div>
