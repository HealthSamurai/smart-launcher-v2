...
function reducer(
  state: PatientPickerState,
  action: PatientPickerAction,
): PatientPickerState {
  switch (action.type) {
    case "setSort":
...
function Gender({ patient }: { patient: fhir4.Patient }) {
  var gender = String(patient.gender || "-")
    .charAt(0)
    .toUpperCase();
  if (gender === "M") return <span className="male">M</span>;
  if (gender === "F") return <span className="female">F</span>;
  return (
    <span style={{ color: "#666" }} title={patient.gender}>
      {gender}
    </span>
  );
}
...
export default function PatientPicker() {
  ...
  const { sortParam, sortDir, pageSize, searchText, skip, baseUrl, patient } =
    state;
  ...
  const { data, loading, error } = useFetch<fhir4.Bundle<fhir4.Patient>>(
    url.href,
    {
      headers: {
  ...
  const endRec = totalRecs
    ? Math.min(skip + pageSize, totalRecs)
    : skip + pageSize;
  ...
  function launchApp(patientId: string) {
    ...
    window.top?.postMessage(
      {
        type: "setPatient",
        payload: data?.entry?.find((rec) => rec.resource?.id === patientId)
          ?.resource,
      },
  ...
  return (
    <HelmetProvider>
      ...
      <div
        className="container-fluid patient-picker"
        style={{ maxWidth: "80em" }}
      >
        {error && (
          <div className="row">
            ...
            <div className="col-sm-12">
              <div className="alert alert-danger">
                ...
                <span>
                  {String(
                    error ||
                      "An error occurred loading patients, please try again later.",
                  )}
                </span>
              </div>
            </div>
        ...
        <div className="row">
          <div className="col-sm-12">
            <table className="table">
              <thead>
                <tr className="no-hover">
                  <th
                    ...
                    onClick={() => sort("name")}
                    onKeyDown={(e) => {
                      if (sortKeys.includes(e.key)) sort("name");
                    }}
                  >
                  ...
                  <th
                    ...
                    onClick={() => sort("gender")}
                    onKeyDown={(e) => {
                      if (sortKeys.includes(e.key)) sort("gender");
                    }}
                  >
                  ...
                  <th
                    ...
                    onClick={() => sort("age")}
                    onKeyDown={(e) => {
                      if (sortKeys.includes(e.key)) sort("age");
                    }}
                  >
              ...
              <tbody>
                {loading && !data && (
                  <tr id="message-loading" className="no-hover">
                    <td
                      colSpan={3}
                ...
                {data && !data.entry?.length && (
                  <tr id="message-no-patients" className="no-hover">
                    <td
                      colSpan={3}
                      style={{ textAlign: "center", padding: 40 }}
                    >
                ...
                {data?.entry?.map((p: fhir4.BundleEntry<fhir4.Patient>, i) => (
                  ...
                  >
                    ...
                    <td className="text-center">
                      <Gender patient={p.resource!} />
                    </td>
                    <td className="text-left">
                      {formatAge(p.resource!)}
                      {(p.resource!.deceasedDateTime ||
                        !!p.resource!.deceasedBoolean) && (
                        <>
                          &nbsp;
                          <span className="label label-warning">deceased</span>
                        </>
                      )}
                    </td>
                  </tr>
