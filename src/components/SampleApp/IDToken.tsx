...
function CountDown({ exp }: { exp: number }) {
  ...
  return (
    <>
      {moment.duration(exp - now, "seconds").humanize(true, { s: 60, m: 60 })}
    </>
  );
}
  ...
  return (
    <div className="panel">
      ...
      <p className="alert alert-info mt-1">
        ...
        <b>id_token</b> is a JWT that comes with the token response and contains
      ...
      {code ? (
            ...
            <table className="table table-condensed table-fixed">
              ...
              <tbody>
                <tr>
                  <td>
                    <b>profile:</b>&nbsp;
                    <span className="text-success">{code.profile}</span>
                    <div className="text-muted small">
                      User identifier (same as <code>fhirUser</code> for
                ...
                <tr>
                  <td>
                    <b>fhirUser:</b>&nbsp;
                    <span className="text-success">{code.fhirUser}</span>
                    <div className="text-muted small">
                      User identifier (same as <code>profile</code> for
                      backwards compatibility)
                    </div>
                  </td>
                </tr>
                <tr>
                  <td>
                    <b>iss:</b>&nbsp;
                    <span className="text-success">{code.iss}</span>
                    <div className="text-muted small">
                ...
                <tr>
                  <td>
                    <b>iat:</b>&nbsp;
                    <span className="text-danger">{code.iat}</span>&nbsp;
                    <span className="text-muted">
                      (
                      <span className="text-info">
                        {new Date(code.iat * 1000).toLocaleString()}
                      </span>
                      )
                    </span>
                    <div className="text-muted small">
                      Time at which the JWT was issued. Its value is a JSON
                ...
                <tr>
                  <td>
                    <b>exp:</b>&nbsp;
                    <span className="text-danger">{code.exp}</span>&nbsp;
                    <span className="text-muted">
                      (
                      <span className="text-info">
                        {new Date(code.exp * 1000).toLocaleString()}
                      </span>
                      )
                    </span>
                    <div className="text-muted small">
                      <Clip
                        txt="Expiration time on or after which the ID Token MUST NOT be
                ...
                <tr>
                  <td>
                    <b>aud:</b>&nbsp;
                    <span className="text-success">{code.aud}</span>
                    <div className="text-muted small">
                      Audience(s) that this ID Token is intended for. It MUST
                      contain the OAuth 2.0 client_id of the Relying Party as an
                      audience value. In this case aud is the{" "}
                      <code>client_id</code> of the app.
                    </div>
                  </td>
                </tr>
                <tr>
                  <td>
                    <b>sub:</b>&nbsp;
                    <span className="text-success">{code.sub}</span>
                    <div className="text-muted small">
                      Subject Identifier. A locally unique and never reassigned
                      identifier within the Issuer for the End-User, which is
                      intended to be consumed by the Client. It MUST NOT exceed
                      255 ASCII characters in length. The sub value is a case
                      sensitive string.
                    </div>
                  </td>
                </tr>
                <tr>
                  <td>
                    ...
                    <div className="text-muted small">
                      <Clip
                        txt="String value used to associate a Client session with an ID
              ...
              <thead>
                ...
                {Object.keys(headers!).map((key, i) => (
                  <tr key={i}>
                    <td>
                      <b>{key}:</b>&nbsp;
                      <span className="text-success">{headers![key]}</span>
                    </td>
                  </tr>
                ))}
              </thead>
        ...
        <div>
          <h5 className="alert alert-danger">
            ...
            No id_token obtained
          </h5>
          <p className="text-muted" style={{ marginLeft: 20 }}>
            <i
              className="glyphicon glyphicon-info-sign text-primary"
              style={{ marginLeft: -20 }}
            />{" "}
            The <code>id_token</code> is only provided to apps using the{" "}
            <b>standalone</b> or <b>patient portal</b> launch types.
          </p>
          <p className="text-muted" style={{ marginLeft: 20 }}>
            <i
              className="glyphicon glyphicon-info-sign text-primary"
              style={{ marginLeft: -20 }}
            />{" "}
            The <code>id_token</code> is only to apps requesting{" "}
            <code>openid</code> and <code>fhirUser</code> or{" "}
