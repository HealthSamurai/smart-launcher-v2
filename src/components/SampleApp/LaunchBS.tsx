      ...
      <div style={{ paddingLeft: 30 }}>
        <details>
          ...
          <pre style={{ whiteSpace: "pre-wrap", padding: "2px 10px" }}>
            <div>
              <b style={{ color: "rgb(170, 13, 145)" }}>GET</b>{" "}
              <span style={{ color: "rgb(131, 108, 40)" }}>
                {url.pathname.replace(/\/sim\/.*?\//, "/sim/...../")} HTTP/1.1
              </span>
            </div>
            <div>
              <b style={{ color: "rgb(170, 13, 145)" }}>Host:</b>{" "}
              <span style={{ color: "rgb(131, 108, 40)" }}>{url.host}</span>
            </div>
            <div>
              <b style={{ color: "rgb(170, 13, 145)" }}>Accept:</b>{" "}
              <span style={{ color: "rgb(131, 108, 40)" }}>
                application/json
              </span>
            </div>
        ...
        <details open={!!error}>
          <summary className={error ? "text-danger" : "text-success"}>
            <b>Response</b>
          </summary>
          {error ? (
            ...
            <SyntaxHighlighter language="json" style={xcode}>
              {JSON.stringify(data || "null", null, 4).replace(
                /\/sim\/.*?\//g,
                "/sim/...../",
              )}
            </SyntaxHighlighter>
...
function LaunchForm({ state, dispatch }: StepProps) {
  ...
  return (
    <details>
      <summary>
        <b>2. Customize Launch (optional)</b>
      </summary>
      <div style={{ paddingLeft: 30 }}>
        <form
          onSubmit={(e) => {
        ...
        >
          <div
            ...
            style={{ display: "block", background: "#F8F8F8" }}
          >
            <div className="row">
              <div className="col-sm-6">
                <div className="form-group">
                  <label className="text-primary" htmlFor="scope">
                    Scope
                  </label>
                  <input
                    id="scope"
                  ...
                  <span className="help-block small">
                    {state.client.scope ? (
                      <>
                        Your client is currently allowed to request{" "}
                        <code>{state.client.scope}</code>. You can try
                        requesting a different scope here to test the "invalid
                        scope" errors.
                      </>
                    ) : (
                      <>
                        You have not specified any scopes in the registration
                        options of the launcher. In this case we grant any scope
                        as if it has been allowed at registration time.
                      </>
                    )}
                  </span>
                </div>
              ...
              <div className="col-sm-6">
                <div className="form-group">
                  <label className="text-primary" htmlFor="client_id">
                    Client ID
                  </label>
                  <input
                    id="client_id"
                  ...
                  <span className="help-block small">
                    {state.client.client_id ? (
                      <>
                        Your registered client_id is{" "}
                        <code>{state.client.client_id}</code>. You can try using
                        a different one here to test the "invalid client_id"
                        error.
                      </>
                    ) : (
                      <>
                        You have not specified any client ID in the registration
                        options of the launcher. In this case we accept any
                        client ID as if it has been registered.
                      </>
                    )}
                  </span>
                </div>
            ...
            {state.client.jwks_url && (
              <div className="row">
                <div className="col-xs-12">
                  <div className="form-group">
                    <label className="text-primary" htmlFor="jwks_url">
                      JWKS URL
                    </label>
                    <input
                      id="jwks_url"
                    ...
                    />
                    <span className="help-block small">
                      Your registered client states that your public keys are at{" "}
                      <code>{state.client.jwks_url}</code>. You can try using
                      different URL here to see what error is received.
                    </span>
                  </div>
                </div>
              </div>
...
function CreateAssertion({ state, dispatch }: StepProps) {
  ...
  useEffect(() => {
    importJWK(PRIVATE_JWT_RS384 as fhirclient.JWK)
      .then((privateKey) =>
        signCompactJws("RS384", privateKey, jwt_headers, jwt_claims),
      )
      .then((assertion) => dispatch({ assertion, accessToken: "" }))
      .catch((e) => setError(e + ""));

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [state.clientId, state.tokenEndpoint, state.jwksUrl]);

  return (
    <details open={!!error}>
      <summary>
        <b>3. Create Client Assertion Token</b>
      </summary>
      <div style={{ paddingLeft: 30 }}>
        <br />
        <b className="text-danger">Pseudo Code:</b>
        <br />
        <br />
...
function GetAccessToken({ state, dispatch }: StepProps) {
  ...
  const { loading, error, data } = useFetch<Record<string, any>>(
    url.href,
    {
      method: "POST",
      body,
    ...
    <details open={!!error}>
      ...
      <div style={{ paddingLeft: 30 }}>
        <details open>
          ...
          <pre style={{ whiteSpace: "pre-wrap", padding: "2px 10px" }}>
            <div>
              <b style={{ color: "rgb(170, 13, 145)" }}>POST</b>{" "}
              <span style={{ color: "rgb(131, 108, 40)" }}>
                {url.pathname.replace(/\/sim\/.*?\//, "/sim/...../")} HTTP/1.1
              </span>
            </div>
            ...
            <div>
              <b style={{ color: "rgb(170, 13, 145)" }}>Content-Type:</b>{" "}
              <span style={{ color: "rgb(131, 108, 40)" }}>
            ...
            <br />
            <div>
              <b style={{ color: "rgb(170, 13, 145)" }}>scope</b>=
              <span style={{ color: "rgb(131, 108, 40)" }}>
                {encodeURIComponent(state.scope)}
              </span>
              <br />
              <b style={{ color: "rgb(170, 13, 145)" }}>client_assertion</b>=
              <span style={{ color: "rgb(131, 108, 40)" }}>
                <Clip txt={encodeURIComponent(state.assertion!)} max={40} />
              </span>
              <br />
              <b style={{ color: "rgb(170, 13, 145)" }}>
                client_assertion_type
              </b>
              =
              <span style={{ color: "rgb(131, 108, 40)" }}>
              ...
              <br />
              <b style={{ color: "rgb(170, 13, 145)" }}>grant_type</b>=
              <span style={{ color: "rgb(131, 108, 40)" }}>
                client_credentials
              </span>
            </div>
...
function FetchPatients({ state }: { state: State }) {
  ...
  return (
    <details open>
      <summary>
        <b>5. Access FHIR API</b> to get a list of Patients{" "}
        {loading && <i className="fa-solid fa-spinner fa-spin"></i>}
      </summary>
      <div style={{ paddingLeft: 30 }}>
        <details>
          ...
          <pre style={{ whiteSpace: "pre-wrap", padding: "2px 10px" }}>
            ...
            <div>
              <b style={{ color: "rgb(170, 13, 145)" }}>Authorization:</b>{" "}
              <span style={{ color: "rgb(131, 108, 40)" }}>
                Bearer <Clip txt={state.accessToken} max={20} />
              </span>
            </div>
          </pre>
          <br />
...
export default function LaunchBS() {
    ...
    return (
      <div className="container">
        ...
        <div className="alert alert-danger">
          An 'aud' URL parameter is required
        </div>
      </div>
    );
  }
  ...
  return (
    <HelmetProvider>
      ...
      <div className="container">
        <h3>
          SMART Backend Service <span className="text-muted">Sample App</span>
        </h3>
        <hr />
        {!client.jwks && !client.jwks_url && (
          <p className="alert alert-warning" style={{ flex: "1 1 100%" }}>
            <i className="glyphicon glyphicon-info-sign" /> This app can be
            authorized using the public keys found at{" "}
            <a
              href="https://www.hl7.org/fhir/smart-app-launch/RS384.public.json"
              target="_blank"
        ...
        {state.tokenEndpoint && (
          <CreateAssertion state={state} dispatch={dispatch} />
        )}
        {state.assertion && (
          <GetAccessToken state={state} dispatch={dispatch} />
        )}
