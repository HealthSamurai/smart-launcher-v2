...
const KNOWN_PROPS = {
  need_patient_banner: {
    ...
    desc: (
      <>
        If <code>false</code>, the app can omit some patient information (like
        name, DOB and age) because that is already displayed within the EHR UI.
      </>
    ),
  },
  smart_style_url: {
    ...
    desc: (
      <>
        Apps can use these style settings to make sure they blend well with the
        appearance of the hosting EHR.
      </>
    ),
  },
  patient: {
  ...
  token_type: {
    ...
    desc: (
      <>
        This should always have the fixed value <code>Bearer</code>.
      </>
    ),
  },
  expires_in: {
  ...
  access_token: {
    ...
    desc: (
      <>
        The access token which is part of the token response. In this server
        this is a JWT but it can be any string elsewhere.
      </>
    ),
  },
  refresh_token: {
      ...
      {mode === "json" ? (
        ...
        <>
          <p className="alert alert-info mt-1">
            <i className="glyphicon glyphicon-info-sign text-primary" />{" "}
            Information extracted from the response of the Token Request
          </p>
                    ...
                    <tr key={i}>
                      <td>
                        ...
                        {value === undefined ? (
                          ...
                          KNOWN_PROPS[k as keyof typeof KNOWN_PROPS].render(
                            value,
                          )
                        ) : (
