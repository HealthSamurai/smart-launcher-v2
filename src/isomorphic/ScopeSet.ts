...
export default class ScopeSet {
  ...
  public static getInvalidSystemScopes(scopes = ""): string {
    ...
    return (
      scopes
        ...
        .find(
          (s) =>
            !/^system\/(\*|[A-Z][a-zA-Z]+)(\.(read|write|\*|[cruds]+))?(\?.*)?$/.test(
              s,
            ),
        ) || ""
    );
   ...
   */
  public negotiate(requested: string | ScopeSet): {
      ...
      if (requestedScope.includes("/") && requestedScope.includes(".")) {
        ...
        if (
          this.scopes.find((s) => {
            ...
            if (
              grantedScope.level !== "*" &&
              grantedScope.level !== scope.level
            ) {
              return false;
            }
            ...
            if (
              grantedScope.resource !== "*" &&
              grantedScope.resource !== scope.resource
            ) {
              return false;
            }
            ...
            if (
              grantedScope.actions.get("create") !== scope.actions.get("create")
            ) {
              return false;
            }
            ...
            if (
              grantedScope.actions.get("read") !== scope.actions.get("read")
            ) {
              return false;
            }
            ...
            if (
              grantedScope.actions.get("update") !== scope.actions.get("update")
            ) {
              return false;
            }
            ...
            if (
              grantedScope.actions.get("delete") !== scope.actions.get("delete")
            ) {
              return false;
            }
            ...
            if (
              grantedScope.actions.get("search") !== scope.actions.get("search")
            ) {
              return false;
            }
