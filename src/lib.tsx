...
export function formatAge(
  patient: fhir2.Patient | fhir3.Patient | fhir4.Patient,
): string {
...
export function defer(fn: () => void, delay = 0) {
  ...
  deferMap.set(
    fn,
    delay === undefined ? requestAnimationFrame(f) : setTimeout(f, delay),
  );
}
...
export function highlight(str: string, stringToFind = "") {
  ...
  while (index > -1) {
    ...
    index = temp
      .toLocaleLowerCase()
      .indexOf(stringToFind.toLocaleLowerCase(), index + replacement.length);
  }
  return temp;
}
