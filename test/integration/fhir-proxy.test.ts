...
FHIR_VERSIONS.forEach(([ver, server]) => {
  describe("FHIR Proxy " + ver, () => {
    ...
    it("Replies with proper content type", async () => {
      ...
      expect(res.headers.get("content-type")).to.match(
        /application\/(fhir\+json|json\+fhir|json)/,
      );
    ...
    it("Replies with formatted JSON for bundles", async () => {
      server.mock("/Patient", {
        status: 200,
        body: JSON.stringify({ a: [1] }, null, 4),
        headers: { "content-type": "application/json" },
      });
    ...
    it("Replies with formatted JSON for single resources", async () => {
      server.mock("/Patient/1", {
        status: 200,
        body: JSON.stringify({ a: 1 }, null, 4),
        headers: { "content-type": "application/json" },
      });
      const txt = await fetch(
        LAUNCHER.baseUrl + "/v/" + ver + "/fhir/Patient/1",
        { headers: { authorization: `Bearer ${ACCESS_TOKEN}` } },
      ).then((r) => r.text());
      expect(txt).to.match(/\n.+/);
    });
    ...
    it("Injects the SMART information in metadata responses", async () => {
      ...
      expect(
        authorizeCfg,
        "No 'authorize' endpoint found in the conformance statement",
      ).to.exist;
      expect(
        authorizeCfg.valueUri,
        "Wrong 'authorize' endpoint found in the conformance statement",
      ).to.equal(LAUNCHER.baseUrl + "/v/" + ver + "/auth/authorize");

      // token -----------------------------------------------------------
      let tokenCfg = uris.find((o) => o.url == "token");
      expect(tokenCfg, "No 'token' endpoint found in the conformance statement")
        .to.exist;
      expect(
        tokenCfg.valueUri,
        "Wrong 'token' endpoint found in the conformance statement",
      ).to.equal(LAUNCHER.baseUrl + "/v/" + ver + "/auth/token");

      // token -----------------------------------------------------------
      let introspectCfg = uris.find((o) => o.url == "introspect");
      expect(
        introspectCfg,
        "No 'introspect' endpoint found in the conformance statement",
      ).to.exist;
      expect(
        introspectCfg.valueUri,
        "Wrong 'introspect' endpoint found in the conformance statement",
      ).to.equal(LAUNCHER.baseUrl + "/v/" + ver + "/auth/introspect");
    });
  ...
  it(ver + " provides .well-known/smart-configuration", async () => {
    const res = await fetch(
      LAUNCHER.baseUrl + "/v/" + ver + "/fhir/.well-known/smart-configuration",
    );
    expect(res.status).to.equal(200);
    expect(res.headers.get("content-type")).to.match(/\bjson\b/);
    const json = await res.json();
    expect(json.authorization_endpoint).to.equal(
      LAUNCHER.baseUrl + "/v/" + ver + "/auth/authorize",
    );
  });

  it(ver + " provides .well-known/openid-configuration", async () => {
    const res = await fetch(
      LAUNCHER.baseUrl + "/v/" + ver + "/fhir/.well-known/openid-configuration",
    );
    expect(res.status).to.equal(200);
    expect(res.headers.get("content-type")).to.match(/\bjson\b/);
    const json = await res.json();
