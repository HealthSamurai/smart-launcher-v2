...
describe("token endpoint", () => {
  it("requires application/x-www-form-urlencoded content type", async () => {
    ...
    await expectOauthError(
      ...
      400,
      "invalid_request",
      "Invalid request content-type header 'text/plain' (must be 'application/x-www-form-urlencoded')",
    );
  });
  ...
  it("rejects unsupported grant_type", async () => {
    ...
    await expectOauthError(
      ...
      'Invalid or missing grant_type parameter "bad_grant_type"',
    );
  });
  ...
  describe("authorization_code flow", () => {
    ...
    it("Requires the 'code' token to include redirect_uri", async () => {
      const code = jwt.sign({ redirect_uri: "" }, config.jwtSecret, {
        expiresIn: "5m",
      });
      const res = await fetchAccessToken({
        code,
      ...
      await expectOauthError(
        ...
        401,
        "invalid_client",
        "The authorization token must include redirect_uri",
      );
    });

    it("Requires the 'code' token to have the same redirect_uri as the redirect_uri parameter", async () => {
      const code = jwt.sign(
        { redirect_uri: "http://whatever1" },
        config.jwtSecret,
        { expiresIn: "5m" },
      );
      const res = await fetchAccessToken({
        code,
        redirect_uri: "http://whatever2",
      });
      await expectOauthError(
        res,
        401,
    ...
    describe("PKCE", () => {
      it("Requires code_challenge_method to be 'S256'", async () => {
        ...
        await expectOauthError(
          ...
          400,
          "invalid_request",
          "Unsupported code_challenge_method 'test'. We support only 'S256'",
        );
      });
      ...
      it("Verifies code_challenge", async () => {
        ...
        const res2 = await fetchAccessToken({
          code: code2,
          redirect_uri: "http://whatever",
          code_verifier: codeVerifier,
        });
        ...
        await expectOauthError(
          ...
          401,
          "invalid_grant",
          /Invalid grant or Invalid PKCE Verifier/,
        );
      });
    });
    ...
    describe("Basic auth", () => {
      it("Can simulate invalid client secret error", async () => {
        ...
        await expectOauthError(
          ...
          401,
          "invalid_client",
          "Simulated invalid client secret error",
        );
      });
      ...
      it("Rejects empty authorization header", async () => {
        ...
        await expectOauthError(
          ...
          401,
          "invalid_request",
          "The authorization header 'Basic' cannot be empty",
        );
      });
      ...
      it("Parses the authorization header", async () => {
        ...
        const res = await fetchAccessToken({
          code,
          redirect_uri: "http://whatever",
          requestOptions: { headers: { authorization: "Basic test" } },
        });
        await expectOauthError(
          res,
          401,
      ...
      it("Validates the client_id", async () => {
        ...
        const res = await fetchAccessToken({
          ...
          requestOptions: {
            headers: {
              authorization:
                "Basic " + Buffer.from("test:pass").toString("base64"),
            },
          },
        });
        await expectOauthError(
          ...
          401,
          "invalid_client",
          "Invalid client_id in the basic auth header",
        );
      });
      ...
      it("Validates the client_secret", async () => {
        ...
        const res = await fetchAccessToken({
          ...
          requestOptions: {
            headers: {
              authorization:
                "Basic " + Buffer.from("whatever:pass").toString("base64"),
            },
          },
        });
        await expectOauthError(
          ...
          401,
          "invalid_client",
          "Invalid client_secret in the basic auth header",
        );
      });
      ...
      it("Requires that client_secret is set in registration options", async () => {
        ...
        await expectOauthError(
          ...
          401,
          "invalid_request",
          "Basic authentication is required for confidential clients",
        );
      });
    });
    ...
    it("Includes a refresh token if 'offline_access' scope is granted", async () => {
      const code = jwt.sign(
        { redirect_uri: "http://whatever", scope: "offline_access" },
        config.jwtSecret,
        { expiresIn: "5m" },
      );
      const res = await fetchAccessToken({
    ...
    it("Includes a refresh token if 'online_access' scope is granted", async () => {
      const code = jwt.sign(
        { redirect_uri: "http://whatever", scope: "online_access" },
        config.jwtSecret,
        { expiresIn: "5m" },
      );
      const res = await fetchAccessToken({
    ...
    it("Token lifetime can be customized via accessTokensExpireIn", async () => {
      const code = jwt.sign(
        { redirect_uri: "http://whatever", accessTokensExpireIn: 10 },
        config.jwtSecret,
      );
      const res = await fetchAccessToken({
        code,
    ...
    it("can refresh using refresh_token", async () => {
      const redirect_uri = "http://whatever";
      const code = jwt.sign(
        { redirect_uri, scope: "offline_access" },
        config.jwtSecret,
      );
      const res = await fetchAccessToken({ code, redirect_uri });
      expect(res.status).to.equal(200);
    ...
    describe("Asymmetric authentication", () => {
      ...
      it("fails with bad client_assertion token", async () => {
        ...
        await expectOauthError(
          ...
          401,
          "invalid_request",
          'Could not decode the "client_assertion" parameter',
        );
      });
      ...
      it("throws if jwtHeaders.typ !== 'JWT'", async () => {
        ...
        const assertion =
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IngifQ.eyJpc3MiOiJ4Iiwic3ViIjoieCIsImF1ZCI6IngiLCJleHAiOjEsImp0aSI6IngiLCJpYXQiOjE2NjU0MDczNTZ9.LXMcMfceSYp3YMQU8Wan4Kly-laQ6_2UI3rDWrPagjA";
        ...
        await expectOauthError(
          ...
          401,
          "invalid_client",
          "Invalid token 'typ' header. Must be 'JWT'.",
        );
      });
      ...
      it("throws on missing 'alg' header", async () => {
        ...
        const assertion =
          "eyJ0eXAiOiJKV1QiLCJraWQiOiJ4In0.eyJpc3MiOiJ4Iiwic3ViIjoieCIsImF1ZCI6IngiLCJleHAiOjEsImp0aSI6IngiLCJpYXQiOjE2NjU0MDczNTZ9.LXMcMfceSYp3YMQU8Wan4Kly-laQ6_2UI3rDWrPagjA";
      ...
      it("throws if iss !== sub", async () => {
        ...
        await expectOauthError(
          ...
          401,
          "invalid_client",
          "The token sub does not match the token iss claim",
        );
      });
      ...
      it("validates aud", async () => {
        ...
        await expectOauthError(
          ...
          401,
          "invalid_client",
          `Invalid token 'aud' value (x). Must be '${LAUNCHER.baseUrl + "/v/r4/auth/token"}'.`,
        );
      });
      ...
      it("throws if jku is not a valid URL", async () => {
        ...
        await expectOauthError(
          ...
          401,
          "invalid_client",
          "Failed to fetch JWKS from b. Only absolute URLs are supported",
        );
      });
      ...
      describe("Remote JWKS validation", () => {
        ...
        it("fails if the jwks url is not found", async () => {
          ...
          await expectOauthError(
            ...
            401,
            "invalid_client",
            `Failed to fetch JWKS from ${JWKS_MOCK_SERVER.baseUrl}/jwks. 404 Not Found`,
          );
        });
        ...
        it("fails if the fetched jwks has no keys property", async () => {
          ...
          await expectOauthError(
            ...
            401,
            "invalid_client",
            'JWKS does not have a "keys" property',
          );
        });
        ...
        it("fails if the fetched jwks keys property is not an empty array", async () => {
          ...
          await expectOauthError(
            ...
            401,
            "invalid_client",
            `No usable keys found`,
          );
        });

        it("fails if the none of the keys have an alg property", async () => {
          const res = await test({
            body: { keys: [{ key_ops: [] }, { key_ops: [] }] },
          });
          await expectOauthError(
            res,
            401,
            "invalid_client",
            `None of the keys found in the JWKS have alg equal to ES384`,
          );
        });

        it("fails if the none of the keys have the needed kid", async () => {
          const res = await test({
            body: { keys: [{ key_ops: [], alg: "ES384" }, { key_ops: [] }] },
          });
          await expectOauthError(
            res,
            401,
            "invalid_client",
            `None of the keys found in the JWKS have kid equal to ${ES384_JWK.kid}`,
          );
        });
        ...
        it("fails if multiple keys match all requirements", async () => {
          ...
          await expectOauthError(
            ...
            401,
            "invalid_client",
            `Multiple usable public keys found in the JWKS`,
          );
        });
        ...
        it("fails if the found key is not valid JWK", async () => {
          const res = await test({
            body: {
              keys: [{ key_ops: ["verify"], alg: "ES384", kid: ES384_JWK.kid }],
            },
          });
          await expectOauthError(
            res,
            401,
            "invalid_client",
            /^No usable public key found in the JWKS\./,
          );
        });

        it("throws if jku is not whitelisted", async () => {
          const res = await test(
            { body: { keys: [{ ...ES384_JWK, key_ops: ["verify"] }] } },
            "a",
          );
          await expectOauthError(
            res,
            401,
            "invalid_request",
            `Invalid jku header of the assertion token token. must be '${JWKS_MOCK_SERVER.baseUrl}/jwks'`,
          );
        });
      });
    });
  ...
  it("rejects token requests with invalid redirect_uri param", async () => {
    ...
    expect(fetchAccessToken({ code, redirect_uri: "http://something.else" })).to
  ...
  describe("Backend Services", () => {
    ...
    it("requires client_assertion_type param", async () => {
      ...
      await expectOauthError(
        ...
        400,
        "invalid_client",
        "Missing 'client_assertion_type' parameter",
      );
    });
    ...
    it("validates client_id", async () => {
      ...
      await expectOauthError(
        ...
        401,
        "invalid_client",
        "Invalid 'client_id' (as sub claim of the assertion JWT)",
      );
    });
    ...
    it("validates scopes", async () => {
      ...
      await expectOauthError(
        ...
        401,
        "invalid_scope",
        'Invalid scope(s) "x" requested. Only system scopes are allowed.',
      );
    });
    ...
    it("discards private keys", async () => {
      ...
      await expectOauthError(
        ...
        401,
        "invalid_client",
        "No usable public keys found in the JWKS",
      );
    });
...
it("The revoke endpoint is N/A for this server", async () => {
  const res = await fetch(LAUNCHER.baseUrl + "/v/r4/auth/revoke", {
    method: "POST",
  });
  expect(res.ok).to.equal(false);
  expect(await res.text()).to.equal(
    "POST /v/r4/auth/revoke is not supported by this server",
  );
});

it("The manage endpoint is N/A for this server", async () => {
  const res = await fetch(LAUNCHER.baseUrl + "/v/r4/auth/manage", {
    method: "POST",
  });
  expect(res.ok).to.equal(false);
  expect(await res.text()).to.equal(
    "POST /v/r4/auth/manage is not supported by this server",
  );
});
