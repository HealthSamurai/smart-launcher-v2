...
export const ACCESS_TOKEN = jwt.sign(
  { client_id: "launcherTests" },
  config.jwtSecret,
);
...
export function launch({
  ...
  if (
    ...
    url = new URL(
      `/v/${fhirVersion}/sim/${launch}/auth/authorize`,
      LAUNCHER.baseUrl,
    );
    if ("aud" in query && query.aud === undefined) {
      ...
      searchParams.set(
        "aud",
        query.aud ?? `${LAUNCHER.baseUrl}/v/${fhirVersion}/sim/${launch}/fhir`,
      );
    }
  }
  ...
  // In EHR launch the launch params are in launch query parameter
    ...
    if ("aud" in query && query.aud === undefined) {
      ...
      searchParams.set(
        "aud",
        query.aud ?? `${LAUNCHER.baseUrl}/v/${fhirVersion}/fhir`,
      );
    }
  }
...
export async function getAccessToken(options: LaunchOptions) {
  ...
  expect(code, "The code param must be non-empty string").to.be.a("string").and
    .to.not.be.empty;
